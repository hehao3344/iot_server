...
LOCAL int ICACHE_FLASH_ATTR transfer_set( struct jsontree_context *js_ctx, struct jsonparse_state *parser )
{
    ...
    while ( ( type = jsonparse_next( parser ) ) != 0 )
    {
        if ( type == JSON_TYPE_PAIR_NAME)
        {
            if ( jsonparse_strcmp_value( parser, "clt_ip" ) == 0 )
            {
                jsonparse_next(parser);
                jsonparse_next(parser);
                os_memset( ip, 0, sizeof( ip ) );
                jsonparse_copy_value( parser, (char*)ip, sizeof( ip ) );
                // debug_print("proxy [device->client] get ip: %s \n", ip );

            }
            else if ( jsonparse_strcmp_value( parser, "clt_port" ) == 0 )
            {
                jsonparse_next(parser);
                jsonparse_next(parser);
                port = jsonparse_get_value_as_int( parser );

                PeerMsgParam *msg = ( PeerMsgParam * )js_ctx->arg;
                //{
                //"clt_ip":"192.168.1.100",
                //"clt_port":8080,
                //"xxxx":"xxxx",
                //...
                //}
                ...
                tmp = os_strstr( msg->msg_body, "\"clt_port\":" );
                if ( NULL != tmp )
                {
                    // "clt_port":12082,
                    tmp = os_strstr( tmp, ",\n" ); // fix me, "\n" "\r\n"
                ...
                {
                    uint16 out_len = MAX_MSG_LEN;
                    if ( msg_handle_packet( msg->buf, &out_len, 2, MSG_MSC_PROXY, msg->tmp_buf  ) )
                    {
...
JSONTREE_OBJECT( transfer_tree,
                 JSONTREE_PAIR( "clt_ip",   &transfer_callback ),
                 JSONTREE_PAIR( "clt_port", &transfer_callback ) );
        ...
        if ( type == JSON_TYPE_PAIR_NAME)
        {
            if ( jsonparse_strcmp_value( parser, "id" ) == 0 )
            {
                ...
                if ( device_mgr_get_proxy_address( handle->hdev_mgr, id, ip, &port ) )
                {
                    ...
                    //...
                    //}
                    //printf("get client address %s:%d \n", ip, port );
                    int8 *tmp = NULL;
                    boolean success = FALSE;
                    //printf("body %s \n", msg->msg_body );
                    tmp = os_strstr( msg->msg_body, "\",\n" ); // fix me, "\n" "\r\n"
                    if ( NULL != tmp )
                    {
                        tmp += strlen( "\",\n" );
                        sprintf( msg->tmp_buf, "{\n\"clt_ip\":\"%s\",\n\"clt_port\":%d,\n%s", msg->peer_ip, msg->peer_port, tmp );
                        success = TRUE;
                    }
                    else
                    {
