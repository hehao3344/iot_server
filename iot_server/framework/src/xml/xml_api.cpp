...
static TiXmlElement* xml_find_element(XML_HANDLE handle, const char *rootString, const char *childString, const char *value);
...
void xml_set_callback(XML_HANDLE handle, xml_save_callback cb, void *arg)
{
    if (NULL == handle)
    {
        return;
...
char *xml_get_string(XML_HANDLE handle, const char *rootString,
                     const char *childString, const char *dftValue)
{
    TiXmlElement* element = xml_find_element(handle, rootString, childString, NULL);

    return (char *)((NULL == element) ? dftValue : (char *)element->Attribute("value"));
}

int xml_get_int(XML_HANDLE handle, const char *rootString, const char *childString, int dftValue)
{
    int value = dftValue;
    TiXmlElement* element = xml_find_element(handle, rootString, childString, NULL);
...
int xml_set_string(XML_HANDLE handle, const char *rootString, const char *childString, const char *value)
{
    int ret = -1;
    TiXmlElement* element = xml_find_element(handle, rootString, childString, value);
    ...
    if (NULL != handle->cb)
    {
        handle->cb(handle->arg, handle->fileName);
    }

    return ret;
}

int xml_set_int(XML_HANDLE handle, const char *rootString, const char *childString, int value)
{
    char buffer[16];
    int ret = -1;
    ...
    TiXmlElement* element = xml_find_element(handle, rootString, childString, buffer);
    if (NULL != element)
    {
        element->SetAttribute("value", value);
        handle->doc->SaveFile();
...
void  xml_destroy(XML_HANDLE handle)
{
    if (NULL != handle)
    {
        //handle->doc->SaveFile();
...
static TiXmlElement* xml_find_element(XML_HANDLE handle, const char *rootString, const char *childString, const char *value)
{
    ...
    rootNode = handle->doc->RootElement();
    ...
    if ((NULL == rootNode) && (NULL != value))
    {
        ...
        TiXmlNode* curRootNode = handle->doc->InsertEndChild(rootText);
...
int xml_unit_test(void)
{
    ...
    XML_HANDLE handle = NULL;

    handle = xml_create("Config.xml");
    if (handle)
    {
        xml_set_int(handle, "Break", "test", 2343);
        xml_destroy(handle);
    }
